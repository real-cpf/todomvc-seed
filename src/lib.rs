...
use std::{convert::TryFrom, u16, usize};
use std::mem;
...
fn update(msg: Msg, model: &mut Model, orders: &mut impl Orders<Msg>) {
    match msg {
        ...
        // ------ Basic Todo operations ------
        Msg::CreateTodo => {
            let  mut  title = model.new_todo_title.trim();
            let maybe_time_str = title.clone();
            // time_str = 30分钟后  1个小时后 1天后 1周后
            let mut time_num: u16 = 0;
            let mut unit: u8 = 0; // 分 1 时 2 天 3 周 4
            // let mut format_type: u8 = 0;
            if maybe_time_str.contains("@") {
                let time_str_arr: Vec<&str > = maybe_time_str.split("@").collect::<Vec<&str>>();
                let time_str=time_str_arr[1];
                title=time_str_arr[0];

                for (i, c) in time_str.char_indices() {
                    if c >= '0' && c <= '9' {
                        time_num = (c as u16)-48+ (i as u16 * 10);
                    } else {
                        let t = c;
                        match t {
                            '时' => {
            ...
            // per array
            let base_arr: [u16; 5] = [0, 1, 60, 1440, 10080];
            let min_num = (time_num * base_arr[unit as usize]) as i64;
            let duration = Duration::minutes(min_num);

            // 现在 只考虑 xx后
            let now = Utc::now();
            let target_time=now.checked_add_signed(duration);
        ...
        todos.map(|todo| {
            ...
            li![C![IF!(todo.completed => "completed"), IF!(is_selected => "editing")],
                ...
                div![C!["view"],
                    ...
                    label![
                        format!("{}=>{}",&todo.title,&todo.dead_line.unwrap().with_timezone(&FixedOffset::east(8*3600)).format("%Y-%m-%d %H:%M")),
